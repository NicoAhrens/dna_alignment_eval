...
# it prints out the two sequences and in between of those two sequences
# the line of evaluations of every pair.
...
def concat_sequnces_in_string(seq1, seq2):
    # TEST: print('This is sequence 1:', seq1)
    # TEST: print('This is the class of sequence 1:', type(seq1))
...
def alignment_eval(seq1, seq2):
    ...
    # TEST: print(sorted_dict)
    wrong_inputs = []
    merged_dictionary_values = \
        [
            'AA', 'GG', 'CC', 'TT', 'CT', 'TC', 'AG', 'GA', 'CA', 'AC', 'CG',
            'GC',
            'TA', 'AT', 'TG', 'GT', 'A_', '_A', 'G_', '_G', 'C_', '_C', 'T_',
            '_T', '__', '  '
        ]
    # seq1_seq2_pair is the variable to iterate over in the loop.
    # It uses each concatenated pair from the concat_list to iterate over

    for seq1_seq2_pair in concat_list:
        # TEST: print('Pair: ', seq1_seq2_pair)
        ...
        if seq1_seq2_pair not in merged_dictionary_values:
            # TEST: print('Pair: ', seq1_seq2_pair)
            # TEST: print('QualityValue: ', merged_dictionary_values)
            evaluate_string += '#'
            wrong_inputs.append(
                'Character combination {} in the following sequence '
                'alignment' 
                ' is not allowed. ' 
...
def show_alignment(seq1, seq2):
    ...
    if len(seq1) == len(seq2):
        ...
        print(
            'Did not align sequences, because they are not the same length.\n')
